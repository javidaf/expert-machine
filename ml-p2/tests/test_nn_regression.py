...
def terrain_test():
    ...
    plot_surface_3d(x_mesh, y_mesh, np.flip(np.rot90(z_pred, k=2), axis=1), title="Predicted Terrain", ax=ax2)
...
def NNRegression():
    ...
    y_pred_grid = scaler_y.inverse_transform(y_pred_grid.reshape(-1, 1)).reshape(x_mesh.shape)

    fig = plt.figure(figsize=(12, 5))
    
    ax1 = fig.add_subplot(121, projection='3d')
    plot_surface_3d(x_mesh, y_mesh, y_true_grid, title="Original Franke Function", ax=ax1)

    ax2 = fig.add_subplot(122, projection='3d')
    plot_surface_3d(x_mesh, y_mesh, y_pred_grid, title="Predicted Franke Function", ax=ax2)
